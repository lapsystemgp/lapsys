    ...
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 mb-6 animate-slide-up">
            ...
            <div className="flex gap-2">
              {[0,1,2,3].map(i => <div key={i} className="skeleton h-9 w-20 rounded-xl" />)}
            </div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 animate-slide-up animation-delay-100">
            <div className="skeleton h-6 w-36 mb-6 rounded" />
            <div className="flex gap-2 mb-6 pb-6 border-b border-gray-100">
              {[0,1,2].map(i => <div key={i} className="skeleton h-9 w-24 rounded-xl" />)}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 mb-6">
          <div className="flex items-start justify-between mb-6">
            <div className="flex-1">
              <h1 className="text-3xl font-bold text-gray-900 mb-3">{lab.name}</h1>
              <div className="flex items-center gap-6 text-gray-600 mb-4">
                <div className="flex items-center gap-1">
                  <Star className="w-5 h-5 fill-yellow-400 text-yellow-400" />
                  <span className="text-xl font-semibold">{lab.rating ?? '—'}</span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pb-6 border-b border-gray-200 mb-6">
            <div>
              ...
              <div className="text-gray-900 font-semibold">{lab.accreditation ?? '—'}</div>
            </div>
            <div>
              <div className="flex items-center gap-2 text-gray-500 mb-2">
              ...
              <div className="text-gray-900 font-semibold">{lab.turnaroundTime ?? '—'}</div>
            </div>
            {lab.phone && (
              <div>
                <div className="flex items-center gap-2 text-gray-500 mb-2">
                ...
                <div className="text-gray-900 font-semibold">{lab.phone}</div>
              </div>
            )}
            {lab.contactEmail && (
              <div>
                ...
                <div className="text-gray-900 font-semibold">{lab.contactEmail}</div>
              </div>
            )}
          </div>
                ...
                <div key={review.id} className="border border-gray-100 rounded-xl p-4">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-900 font-semibold">{review.patientName}</span>
                    </div>
                    <div className="flex items-center gap-1">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredTests.map((test) => (
              <div key={test.id} className="border border-gray-100 rounded-2xl p-6 hover:border-blue-300 transition">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="text-xl font-bold text-gray-900">{test.name}</h3>
                      <div className="text-right ml-4">
                        <div className="text-2xl font-bold text-blue-600">EGP {test.priceEgp}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-4">
                      ...
                      <div>
                        <div className="text-gray-500 font-medium mb-1">Turnaround</div>
                      ...
                      <div>
                        <div className="text-gray-500 font-medium mb-1">Parameters</div>
                        <div className="text-gray-900 font-semibold">{test.parametersCount ?? '—'} parameters tested</div>
                      </div>
                    </div>
                  </div>
                </div>
